...
//        $data['messages'] = ContatMall::select('*')->get();
        $data['messages'] = DB::table('contact_mail')
                             ->leftJoin('country', 'country.country_id', '=', 'contact_mail.country_id')
                             ->leftJoin('categories', 'categories.category_id', '=', 'contact_mail.category_id')
                             ->orderBy('id', 'desc')
                            ->get();
//        dd($data['messages'] );
